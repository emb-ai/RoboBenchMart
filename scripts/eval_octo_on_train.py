...
def _main(args, proc_id: int = 0, start_seed: int = 0) -> str:
    ...
    json_scenes_path = scene_dir + '/demos/motionplanning'
    json_path = sorted(list(Path(json_scenes_path).glob('*.json')))[0]
    with open(str(json_path), 'r') as f:
        scenes_data = json.load(f)

    env = gym.make(scenes_data['env_info']['env_id'], 
                    robot_uids=scenes_data['env_info']['env_kwargs']['robot_uids'],
                   config_dir_path = scene_dir,
                   num_envs=1, 
                   control_mode=scenes_data['env_info']['env_kwargs']['control_mode'],
                   viewer_camera_configs=scenes_data['env_info']['env_kwargs']['viewer_camera_configs'], 
                    human_render_camera_configs=scenes_data['env_info']['env_kwargs']['human_render_camera_configs'],
                    sensor_configs=scenes_data['env_info']['env_kwargs']['sensor_configs'],
                #    render_mode="human" if gui else "rgb_array", 
                   render_mode=scenes_data['env_info']['env_kwargs']['render_mode'], 
                   enable_shadow=scenes_data['env_info']['env_kwargs']['enable_shadow'],
                   obs_mode=scenes_data['env_info']['env_kwargs']['obs_mode'],
    ...
    print(f"Octo evaluation on {scenes_data['env_info']['env_id']}")
